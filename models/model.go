...
type User struct {
	...
	Power int `orm:"default(0)"` //权限设置 0 表示未激活  1 表示激活
	Address []*Address  `orm:"reverse(many)"` //多个地址
...
type Address struct {
	...
	Isdefault int	 `orm:"default(0)"`			//是否默认 0 为非默认  1为默认
...
func init()  {
	//注册数据库
	orm.RegisterDataBase("default","mysql","root:123456@tcp(127.0.0.1:3306)/ttsx")
	//注册表
